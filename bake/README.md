...
| `R_DEV_DEPS` | `false` | `r-base`, `rstudio` | `true`, `false`, `1`, `0` | Installs R package development system dependencies, `qpdf` and `ghostscript-x` for package checks, and preinstalls `devtools`, `BiocManager`, `remotes`, and `languageserver` into the R site library. The package install step fails the build if the site library is not writable or any required package is missing after installation. Also forces Java installation during the same image build. In `r-base`, `R_BASE_MODE=dev` forces this behavior even if `R_DEV_DEPS=false`. |
